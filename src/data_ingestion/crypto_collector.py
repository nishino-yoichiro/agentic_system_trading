...
class CryptoDataCollector:
    ...
    def _save_to_raw_data(self, symbol: str, df: pd.DataFrame):
        """Save data to raw data directory for consolidation"""
        try:
            raw_file = Path("data/raw") / f"prices_{symbol}.parquet"
            raw_file.parent.mkdir(parents=True, exist_ok=True)
            df.to_parquet(raw_file)
            logger.debug(f"Saved {len(df)} data points for {symbol} to raw data")
        except Exception as e:
        ...
        for i, symbol in enumerate(symbols, 1):
            try:
                ...
                for gap_idx, (gap_start, gap_end) in enumerate(gaps):
                    ...
                    for request_count in range(gap_requests):
                        ...
                        if candles:
                            ...
                            logger.info(f"  Got {len(candles)} candles, total so far: {len(all_new_candles)}")
                        else:
                ...
                if all_new_candles:
                    ...
                    new_df['timestamp'] = pd.to_datetime(new_df['timestamp'], unit='s', utc=True)
                    ...
                    # Save new data to historical database
                    self._save_historical_data(symbol, new_df)
                    
                    # Get the complete dataset (existing + new) for the requested time range
    ...
    async def save_crypto_data(self, data: Dict[str, pd.DataFrame], data_dir: Path):
        """Save crypto data to parquet files"""
